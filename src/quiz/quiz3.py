...
def train(trn_data: List[List[Tuple[str, str]]], dev_data: List[List[Tuple[str, str
]]]) -> Tuple:
    """
        ...
        for second_weight in grid:
            for third_weight in grid:
                for fourth_weight in grid:
                    args = (cw_dict, pp_dict, pw_dict, nw_dict, first_weight, second_weight, third_weight, fourth_weight)
                    acc = evaluate(dev_data, *args)
                    print('{:5.2f}% - cw: {:3.1f}, pp: {:3.1f}, pw: {:3.1f}, nw: {:3.1f}'.format(acc, first_weight,
                                                                                                 second_weight,
                                                                                                 third_weight,
                                                                                                 fourth_weight))
...
def predict(tokens: List[str], *args) -> List[Tuple[str, float]]:
    ...
    """
    cw_dict, pp_dict, pw_dict, nw_dict, first_weight, second_weight, third_weight, fourth_weight = args
    output = []
    ...
    for i in range(len(tokens)):
        ...
        for pos, prob in cw_dict.get(curr_word, list()):
            scores[pos] = scores.get(pos, 0) + prob * 1
